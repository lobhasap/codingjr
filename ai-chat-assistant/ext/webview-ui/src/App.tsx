...
function App() {
  ...
  function hasAtFilename(text: string) {
    return /@\w+/.test(text);
  }
  ...
  return (
    <div className="chat-container">
      <div className="chat-history">
            ...
            {msg.attachment && (
              <div className="attachment">
                ...
                {msg.attachment.type.startsWith('image/') ? (
                  <img src={`data:${msg.attachment.type};base64,${msg.attachment.content}`} alt={msg.attachment.name} style={{ maxWidth: 200, display: 'block', marginTop: 8 }} />
                ) : (
                  <pre style={{ maxHeight: 200, overflow: 'auto', background: '#f6f8fa', marginTop: 8 }}>{atob(msg.attachment.content).slice(0, 500)}</pre>
                )}
              </div>
            )}
          </div>
        ...
        {loading && (
          <div className="chat-message ai"><em>AI is typing…</em></div>
        )}
      ...
      <div className="chat-input-row">
        <input
          ...
          placeholder="Type a message... (@filename to attach)"
